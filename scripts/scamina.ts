...
if (!RPC_URL || !CONTRACT_ADDR || !USER_ADDRESS) {
  console.error(
    "❌ It is necessary to set in .env: RPC_URL, CONTRACT_ADDRESS, USER_ADDRESS"
  );
  process.exit(1);
}
...
const ABI = [
  ...
  "function pendingProfitOf(address user) view returns (uint256)",
  "function getAvgEntryPrice(address user) view returns (uint256)",
...
async function getPendingProfit(user: string) {
  const raw = (await contract.pendingProfitOf(user)) as bigint;
  console.log(
    `• Pending profit of ${user}:`,
    ethers.formatUnits(raw, 18),
    "USDT"
  );
}

async function getAvgEntryPrice(user: string) {
  const raw = (await contract.getAvgEntryPrice(user)) as bigint;
  console.log(
    `• Avg entry price of ${user}:`,
    ethers.formatUnits(raw, 18),
    "USDT/MMM"
  );
}
...
getBalance("0xf92769A0dFee5B4807daC7De454a0AE009886Fb0").catch((e) => {
  console.error(e);
  process.exit(1);
});
getPrice().catch((e) => {
...
getPendingProfit(USER_ADDRESS).catch((e) => {
  console.error(e);
  process.exit(1);
});
getAvgEntryPrice(USER_ADDRESS).catch((e) => {
  console.error(e);
  process.exit(1);
});
getUsdt().catch((e) => {
...
getTotalBorrow().catch((e) => {
  console.error(e);
  process.exit(1);
});
getTotalStable().catch((e) => {
  console.error(e);
  process.exit(1);
});
